<?php

namespace Samarete\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;

use Samarete\Repositories\UserRepository;
use Samarete\Repositories\RichiestaRepository;

use Samarete\Models\Richiesta;

class RichiestaRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }
    
    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            'id' => 'required|integer|exists:richiesta,id',
        ];
    }
    
    public function richiesta()
    {
        if(empty($this->id)) return Richiesta::class;
        return RichiestaRepository::getById($this->id);
    }

}